...
const protect = asyncHandler(async (req, res, next) => {
    ...
    if (token) {
        try {
            const decoded = jwt.verify(token, process.env.JWT_SECRET);  // decode the token  (in this case the userId)
...
const admin = (req, res, next) => {
    if (req.user && req.user.isAdmin) {  // if user exists and if user is admin
        next();  // go to the next middleware
    } else {
        res.status(401);  // 401 ==> unauthorized access
        throw new Error('Not authorized as an admin. Only admins can access this route');
    }
